...
class Similarity2DiversityMetric(DiversityMetric):
    ...
    def __call__(self, response_set):
        ...
        if not self.batched:
            ...
            for i in range(n):
                for j in range(i):
                    sim = self.similarity_metric(response_set[i], response_set[j])
                    # 裁剪相似度到 [0,1] 范围
                    clipped_sim = max(0.0, min(1.0, sim))
                    similarity_list.append(clipped_sim)
        else:
            pairs = []
            for i in range(n):
                for j in range(i):
            ...
            for i in range(0, len(pairs), self.batch_size):
                ...
                batch_sims = self.similarity_metric(
                    [pair[0] for pair in batch_pairs],
                    [pair[1] for pair in batch_pairs]
                )
                # 裁剪相似度值
                similarity_list.extend([max(0.0, min(1.0, sim)) for sim in batch_sims])
...
class AveragedNgramDiversityMetric(DiversityMetric):
    ...
    def __call__(self, response_set):
        ...
        for n in range(self.config["n_min"], self.config["n_max"] + 1):
            ...
            tmp_config["n"] = n
            ngram_metric = self.ngram_metric.__class__(tmp_config)
            result = ngram_metric(response_set)
            ngrams_results.append(result)
