...
class Scorer(object):
  ...
  def read_gold_file(self, file_path, confidence=False):
    ...
    def process_example(json_blob):
      ...
      if confidence:
        gold_evt_links = [(a, b, c.lower(), 0) for a, b, c in gold_evt_links]
  ...
  def evaluate(self, constrained_decoding=True):
    ...
    unseen_roles = self.read_unseen_arguments(self.args.unseen_role_file)
    ...
    unseen_correct = {category: global_correct.get(category, 0) for category in unseen_roles}
    unseen_missing = {category: global_missing.get(category, 0) for category in unseen_roles}
    unseen_overpred = {category: global_overpred.get(category, 0) for category in unseen_roles}


    all_categories = set(global_correct.keys()) | set(global_missing.keys()) | set(global_overpred.keys())
    seen_roles = all_categories - set(unseen_roles)


    seen_correct = {category: global_correct.get(category, 0) for category in seen_roles}
    seen_missing = {category: global_missing.get(category, 0) for category  in seen_roles}
    seen_overpred = {category: global_overpred.get(category, 0) for category  in seen_roles}

    precision, recall, f1, _ = util.compute_from_counters(global_correct, 
                                                          global_missing,
                                                          global_overpred)
    unseen_precision, unseen_recall, unseen_f1, unseen__ = util.compute_from_counters(unseen_correct,  
                                                          unseen_missing,
                                                          unseen_overpred)
    
    seen_precision, seen_recall, seen_f1, seen__ = util.compute_from_counters(seen_correct,  
                                                          seen_missing,
                                                          seen_overpred)
    distance_metrics = []
    for i in range(5):
    ...
    self.unseen_metrics = {'precision': unseen_precision,
                    'recall': unseen_recall,
                    'f1': unseen_f1}
    
    self.seen_metrics = {'precision': seen_precision,
                    'recall': seen_recall,
                    'f1': seen_f1}
    print('unseen')
    print(self.unseen_metrics)

    print('seen')
    print(self.seen_metrics)
...
if __name__ == "__main__":
  ...
  parser.add_argument('-g', '--gold_file', type=str,default='/root/siton-data-Hzhang/ZGJ/2025/llm_zero_eae/data/wiki2wiki/test.jsonl',
                      help='Gold file path')
  parser.add_argument('-p', '--pred_file', type=str, default='experiments/wiki2wiki/output_llama/extractor/iter0/span_output.jsonl',
                      help='Predictions file path')
  parser.add_argument('--reuse_gold_format', dest='reuse_gold_format',default=True,
                 action='store_true',
                      help="Reuse gold file format for pred file.")
  parser.add_argument('-t', '--ontology_file', type=str,
  ...
  parser.add_argument('--do_all', dest='do_all', default=True,
  ...
  parser.add_argument('--task', dest='task_name',default='wiki2wiki')
  parser.add_argument('--unseen_role', dest='unseen_role_file',default='unseen_role_wiki2wiki')
